...
from app.database import motor_kfk_work_jobs_coll
import logging
...
async def send_jobs(message: str):
    """
    Send a message to all connected WebSocket clients for jobs.
    """
    ...
    for connection in stream_connections[:]:
        try:
            ...
            try:
                await remove_connection_jobs(connection)
...
async def remove_connection_jobs(websocket: WebSocket):
    """
...
@router.websocket("/ws/stream_jobs")
async def websocket_stream_jobs(websocket: WebSocket):
    """
    WebSocket endpoint to stream jobs updates.
    """
    ...
    try:
        ...
        async with motor_kfk_work_jobs_coll.watch(pipeline, full_document='updateLookup') as change_stream:
            async for change in change_stream:
                if websocket.client_state.name == "DISCONNECTED":  # Check if the WebSocket is still open
                    break
                document = change.get("fullDocument", {})
                if document:
                    await send_jobs(dumps(document))
        ...
        await remove_connection_jobs(websocket)
